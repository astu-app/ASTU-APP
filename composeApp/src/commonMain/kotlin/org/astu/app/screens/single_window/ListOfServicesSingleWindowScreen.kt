...
import org.astu.app.components.card.Description
import org.astu.app.components.card.Title
import org.astu.app.components.carousel.Carousel
import org.astu.app.components.searchbar.SearchBar

data class Category(val name: String, val items: MutableList<Item> = mutableListOf())

class ListOfServicesSingleWindowScreen(
    val onAddFromListScreen: (ServiceScreen) -> Unit,
    val onAddScreen: (ServiceScreen) -> Unit,
) : Screen {
    ...
    override fun Content() {
        SearchBar(Modifier.fillMaxWidth().padding(horizontal = 15.dp, vertical = 10.dp), "", {}, CircleShape)
        Divider()
        ...
        LazyColumn {
            categories.forEach { category ->
                ...
                items(category.items) {
                    ...
                    Divider(modifier = Modifier.padding(start = 20.dp, end = 20.dp), thickness = 2.dp)
                }
            }
        }
    }
    ...
    fun ListItem(modifier: Modifier, item: Item) {
        Card(
            modifier.padding(horizontal = 10.dp, vertical = 10.dp).clickable {
                onAddFromListScreen(PrimitiveServiceScreen(item, onAddScreen))
            },
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Column(Modifier.padding(start = 10.dp)) {
                    Title(item.name)
                    Description(item.description, fontSize = 12.sp)
                }
